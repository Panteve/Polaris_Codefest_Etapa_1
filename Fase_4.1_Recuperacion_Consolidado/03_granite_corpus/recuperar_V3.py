"""Recuperacion V3 independiente para Granite."""
...
MODEL_NAME="ibm-granite/granite-embedding-311m-multilingual-r2"; RERANKER_NAME="BAAI/bge-reranker-v2-m3"; FOLDER=Path(__file__).resolve().parent; NEAR_DUPLICATE_THRESHOLD=0.90; RERANK_BATCH_SIZE=8; DEFAULT_QUESTIONS_PATH=FOLDER.parent/"Validacion_20_Preguntas"/"ground_truth_15_preguntas.json"
...
def progress(args,stage,percent): print(f"Pregunta {args._question_number}/{args._question_total} - {stage}: {percent}%",flush=True)
def rerank_chunks(question,texts,reranker,args):
    scores=[]; total=len(texts)
    for start in range(0,total,args.rerank_batch_size):
        batch=texts[start:start+args.rerank_batch_size]; scores.extend(float(value) for value in reranker.predict([(question,text) for text in batch],show_progress_bar=False)); processed=min(start+len(batch),total); print(f"Pregunta {args._question_number}/{args._question_total} - reranking chunks: {int(processed*100/total)}% ({processed}/{total})",flush=True)
    return scores
def encode_chunks(texts,encoder,args):
    vectors=[]; total=len(texts)
    for start in range(0,total,args.rerank_batch_size):
        batch=texts[start:start+args.rerank_batch_size]; vectors.extend(encoder.encode(batch,convert_to_numpy=True,normalize_embeddings=True,show_progress_bar=False)); processed=min(start+len(batch),total); print(f"Pregunta {args._question_number}/{args._question_total} - embeddings MMR: {int(processed*100/total)}% ({processed}/{total})",flush=True)
    return vectors
def deduplicate(scored,vectors,args):
    ...
    for i in order:
        ...
        if chunk_id in exact or (text and text in exact): continue
        if any(float(vectors[i]@vectors[j])>=args.near_duplicate_threshold for j in kept): continue
...
def retrieve(question,encoder,reranker,index,records,args):
    ...
    progress(args,"recall FAISS",25); progress(args,"filtros",40)
    if candidates:
        texts=[record.get("texto","") for _,_,record in candidates]; values=rerank_chunks(question,texts,reranker,args); vectors=encode_chunks(texts,encoder,args); scored=[(values[i],(candidates[i][0],candidates[i][2])) for i in range(len(candidates))]; progress(args,"reranking",60); unique=deduplicate(scored,vectors,args); progress(args,"deduplicacion",75); final=select_mmr([item for item,_ in unique],[vector for _,vector in unique],args) if unique else []; progress(args,"MMR",90)
    else: final=[]
    grouped={}
    ...
    progress(args,"resultado listo",100); return {"query_id":"q001","query":question,"documents":documents,"fragments":fragments}
def load_questions(path):
    ...
    return result
def main():
    parser=argparse.ArgumentParser(description="Recuperacion Granite V3 con deduplicacion, reranking, MMR y mean pooling."); parser.add_argument("--query"); parser.add_argument("--questions",type=Path,default=DEFAULT_QUESTIONS_PATH,help="Archivo JSON con un arreglo de preguntas."); parser.add_argument("--recall-top-k",type=int,default=100); parser.add_argument("--output-top-k",type=int,default=10); parser.add_argument("--model",default=MODEL_NAME); parser.add_argument("--reranker-model",default=RERANKER_NAME); parser.add_argument("--rerank-batch-size",type=int,default=RERANK_BATCH_SIZE); parser.add_argument("--mmr-lambda",type=float,default=0.7); parser.add_argument("--near-duplicate-threshold",type=float,default=NEAR_DUPLICATE_THRESHOLD); parser.add_argument("--device",default=None); parser.add_argument("--index",type=Path,default=FOLDER/"index.faiss"); parser.add_argument("--metadata",type=Path,default=FOLDER/"metadata.jsonl"); parser.add_argument("--output-json",type=Path,default=FOLDER/"resultado_v3.json"); parser.add_argument("--output",type=Path); parser.add_argument("--threshold",type=float,default=-1.0); parser.add_argument("--fenomeno",type=int,choices=(1,2,3)); parser.add_argument("--idioma"); parser.add_argument("--formato"); args=parser.parse_args()
    if args.recall_top_k<1 or args.output_top_k<1 or args.rerank_batch_size<1 or not 0<=args.mmr_lambda<=1 or not 0<=args.near_duplicate_threshold<=1: raise ValueError("Parametros invalidos.")
    ...
    encoder,reranker=SentenceTransformer(args.model,device=args.device),CrossEncoder(args.reranker_model,device=args.device); questions=[("q001",args.query)] if args.query else load_questions(args.questions); output_handle=args.output.open("w",encoding="utf-8") if args.output else None; results=[]
    args._question_total=len(questions)
    try:
        for number,(query_id,question) in enumerate(questions,1):
            args._question_number=number
