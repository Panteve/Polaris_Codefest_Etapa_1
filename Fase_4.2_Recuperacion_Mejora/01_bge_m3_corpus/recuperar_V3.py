...
def progress(args, stage, percent):
    print(f"Pregunta {args._question_number}/{args._question_total} - {stage}: {percent}%", flush=True)

def rerank_chunks(question, texts, reranker, args):
    scores = []
    total = len(texts)
    for start in range(0, total, args.rerank_batch_size):
        batch = texts[start:start + args.rerank_batch_size]
        scores.extend(float(value) for value in reranker.predict([(question, text) for text in batch], show_progress_bar=False))
        processed = min(start + len(batch), total)
        percent = int(processed * 100 / total)
        print(f"Pregunta {args._question_number}/{args._question_total} - reranking chunks: {percent}% ({processed}/{total})", flush=True)
    return scores
...
def encode_chunks(texts, encoder, args):
    ...
    for start in range(0, total, args.rerank_batch_size):
        batch = texts[start:start + args.rerank_batch_size]
        vectors.extend(encoder.encode(batch, convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=False))
        processed = min(start + len(batch), total)
        print(f"Pregunta {args._question_number}/{args._question_total} - embeddings MMR: {int(processed * 100 / total)}% ({processed}/{total})", flush=True)
    return vectors
...
def retrieve(question, encoder, reranker, index, records, args):
    ...
    progress(args, "resultado listo", 100)
