"""Celda Kaggle completa para el corpus 01."""
...
INPUT=Path('/kaggle/input'); OUTPUT=Path('/kaggle/working'); HINT='01_bge_m3_corpus'
...
def locate():
    indexes=files('index.faiss'); metas=files('metadata.jsonl'); pairs=[(i,m) for i in indexes for m in metas if i.parent==m.parent]
    if not pairs: raise FileNotFoundError('No se encontro index.faiss junto a metadata.jsonl.')
    preferred=[p for p in pairs if HINT.casefold() in str(p[0].parent).casefold()]
    pair=(preferred or pairs)[0]; print('index:',pair[0]); print('metadata:',pair[1]); return pair
def question_file():
...
def questions(path):
    value=json.loads(path.read_text(encoding='utf-8')); out=[]
    for n,item in enumerate(value,1): out.append((str(item.get('query_id',f'q{n:03d}')),item['query']))
    return out
def norm(x):
    lo,hi=min(x),max(x); return [1.0]*len(x) if lo==hi else [(v-lo)/(hi-lo) for v in x]
def run(q,qn,total,enc,rer,index,recs):
    print(f'Pregunta {qn}/{total} - inicio: 0%'); v=enc.encode([q],convert_to_numpy=True,normalize_embeddings=True).astype('float32'); s,pos=index.search(v,min(RECALL_TOP_K,index.ntotal)); print(f'Pregunta {qn}/{total} - recall: 25%'); cand=[(r,float(sc),recs[int(p)]) for r,(sc,p) in enumerate(zip(s[0],pos[0]),1) if 0<=p<len(recs)]; print(f'Pregunta {qn}/{total} - filtros: 40%')
    texts=[x[2].get('texto','') for x in cand]; rs=[]; vec=[]
    for a in range(0,len(texts),BATCH_SIZE):
        b=texts[a:a+BATCH_SIZE]; rs += [float(x) for x in rer.predict([(q,t) for t in b],show_progress_bar=False)]; z=min(a+len(b),len(texts)); print(f'Pregunta {qn}/{total} - reranking chunks: {z*100//len(texts)}% ({z}/{len(texts)})')
        vec += list(enc.encode(b,convert_to_numpy=True,normalize_embeddings=True,show_progress_bar=False)); print(f'Pregunta {qn}/{total} - embeddings MMR: {z*100//len(texts)}% ({z}/{len(texts)})')
...
def main():
    ip,mp=locate(); qp=question_file(); recs=metadata(mp); qs=questions(qp); index=faiss.read_index(str(ip)); assert index.ntotal==len(recs); enc=SentenceTransformer(MODEL); rer=CrossEncoder(RERANKER); out=[]
